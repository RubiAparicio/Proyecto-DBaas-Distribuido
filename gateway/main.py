...
class GatewayServidor(puerta_enlace_pb2_grpc.ServicioGatewayServicer):

    def Login(self, request, context):
        print(f"[Gateway] Reenviando intento de login para: {request.usuario}")
        with grpc.insecure_channel('localhost:50053') as canal:
            ...
            return puerta_enlace_pb2.RespuestaLoginGateway(
    ...
    def Registro(self, request, context):
        print(f"[Gateway] Reenviando registro para: {request.usuario} creado por {request.creador}")
        with grpc.insecure_channel('localhost:50053') as canal:
            cliente_auth = seguridad_pb2_grpc.ServicioSeguridadStub(canal)
            # NUEVO: Pasamos el creador a seguridad
            ...
            return puerta_enlace_pb2.RespuestaRegistroGateway(exito=res_auth.exito, mensaje=res_auth.mensaje)
    ...
    def EnviarComando(self, request, context):
        with grpc.insecure_channel('localhost:50053') as canal_auth:
            ...
            if not res_validar.valido:
                return puerta_enlace_pb2.RespuestaComandoGateway(exito=False, mensaje="[ERROR] Token invalido o expirado.", resultado="")
    ...
    def _procesar_admin(self, token, accion, base_datos, tabla, usuario):
        with grpc.insecure_channel('localhost:50052') as canal_meta:
            ...
            return puerta_enlace_pb2.RespuestaComandoGateway(exito=res_meta.exito, mensaje=res_meta.mensaje, resultado=res_meta.datos if accion == "listar_dbs" else "")
    ...
    def _procesar_crud(self, token, base_datos, tabla, accion, datos_json, usuario, rol):
        if rol == "usuario_lectura" and accion not in ["buscar", "join"]:
            return puerta_enlace_pb2.RespuestaComandoGateway(exito=False, mensaje="[ERROR] Tu rol de lectura no permite modificaciones.", resultado="")
        ...
        with grpc.insecure_channel('localhost:50052') as canal_meta:
            ...
            if base_datos not in catalogo:
                return puerta_enlace_pb2.RespuestaComandoGateway(exito=False, mensaje="[ERROR] La base de datos no existe.", resultado="")
                
            propietario = catalogo[base_datos]["propietario"]
            if propietario != usuario and rol != "administrador":
                return puerta_enlace_pb2.RespuestaComandoGateway(exito=False, mensaje=f"[ERROR] Acceso denegado. Propiedad de {propietario}.", resultado="")

            if accion not in ["crear_db"] and tabla not in catalogo[base_datos]["tablas"]:
                return puerta_enlace_pb2.RespuestaComandoGateway(exito=False, mensaje="[ERROR] La tabla origen no existe.", resultado="")

            # Validacion adicional si la accion es un INNER JOIN relacional
            if accion == "join":
                ...
                if "_" in t2_completa:
                    ...
                    if db2_chk not in catalogo or t2_chk not in catalogo[db2_chk]["tablas"]:
                        return puerta_enlace_pb2.RespuestaComandoGateway(exito=False, mensaje="[ERROR] La segunda tabla especificada en el JOIN no existe.", resultado="")
        ...
        with grpc.insecure_channel('localhost:50051') as canal_storage:
            ...
            return puerta_enlace_pb2.RespuestaComandoGateway(exito=res_db.exito, mensaje=res_db.mensaje, resultado=res_db.resultado)
    ...
    def _procesar_agregacion(self, token, base_datos, tabla, operacion, campo, usuario, rol):
        with grpc.insecure_channel('localhost:50052') as canal_meta:
            ...
            if base_datos not in catalogo or tabla not in catalogo[base_datos]["tablas"]:
                return puerta_enlace_pb2.RespuestaComandoGateway(exito=False, mensaje="[ERROR] Estructura no valida.", resultado="")
                
            propietario = catalogo[base_datos]["propietario"]
            if propietario != usuario and rol != "administrador":
                return puerta_enlace_pb2.RespuestaComandoGateway(exito=False, mensaje="[ERROR] No tienes acceso a esta base de datos.", resultado="")
        ...
        with grpc.insecure_channel('localhost:50054') as canal_mpi:
            ...
            return puerta_enlace_pb2.RespuestaComandoGateway(exito=res_mpi.exito, mensaje=res_mpi.mensaje, resultado=res_mpi.resultado)
    ...
    def _ejecutar_parser_sql(self, comando, token, usuario, rol):
        ...
        return puerta_enlace_pb2.RespuestaComandoGateway(exito=False, mensaje="[ERROR] Sintaxis SQL no reconocida.", resultado="")
    ...
    def _ejecutar_parser_json(self, comando, token, usuario, rol):
        try:
            ...
            return puerta_enlace_pb2.RespuestaComandoGateway(exito=False, mensaje=f"[ERROR] JSON invalido: {str(e)}", resultado="")

def iniciar():
    servidor = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    puerta_enlace_pb2_grpc.add_ServicioGatewayServicer_to_server(GatewayServidor(), servidor)
    servidor.add_insecure_port('[::]:50050')
    print("[INFO] Puerta de Enlace (Gateway) activa como servidor en el puerto 50050...")
    servidor.start()
